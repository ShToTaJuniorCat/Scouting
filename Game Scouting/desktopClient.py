...
def first_level_pieces_by_game(game_num):
    ...
    auto_first_level = game["au_Collected pieces count"]
...
def graph_total_collected_from_json(game_json):
    auto_collected_pieces = game_json["au_Collected pieces count"]
...
for key in keys:
    all_games_json[re.split("(\d+)", key)[1]] = all_games_json.pop(key)
